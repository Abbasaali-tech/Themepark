...
template <typename T>
T *findByIdMutable(vector<T> &items, int id)
{
    auto it = find_if(items.begin(), items.end(), [id](const T &item)
                      { return item.getId() == id; });
...
public:
    ...
    void setVisitorId(int newVisitorId) { visitorId = newVisitorId; }
    void setRideId(int newRideId) { rideId = newRideId; }
...
public:
    ...
    void setCostPerItem(double newCost) { costPerItem = newCost; }
...
private:
    ...
    void managerPortal()
    {
        ...
        while (true)
        {
            ...
            cout << "5. Update Ride\n";
            cout << "6. Update Event\n";
            cout << "7. Update Inventory\n";
            cout << "8. Update Staff\n";
            cout << "9. Update Visitor\n";
            cout << "10. Update Ticket\n";
            cout << "11. View Feedback\n";
            cout << "12. View Revenue Report\n";
            cout << "13. View All Staff\n";
            cout << "14. View All Rides\n";
            cout << "15. View All Events\n";
            cout << "16. View All Inventory\n";
            cout << "17. Save Data\n";
            cout << "18. Logout\n\n";

            int choice = readNumber<int>("Enter choice: ", 1, 18);
...
private:
    ...
    void updateRide()
    {
        if (rides.empty())
        {
            cout << "No rides available to update.\n";
            return;
        }

        listRides();
        int rideId = readNumber<int>("Enter Ride ID to update: ", 1, 1000000);
        Ride *ride = findByIdMutable(rides, rideId);
        if (!ride)
        ...
        cout << "4. Update All\n\n";
        int choice = readNumber<int>("Enter choice: ", 1, 4);

        if (choice == 1 || choice == 4)
        {
            string newName;
            cout << "Enter new ride name: ";
            getline(cin, newName);
        ...
        {
            double newPrice = readNumber<double>("Enter new ride price: ", 0.0, 10000.0);
        ...
        {
            int statusChoice = readNumber<int>("Status (1 Operational, 2 Maintenance): ", 1, 2);
            ride->setOperational(statusChoice == 1);
        }

        ConsoleColor::printSuccess("✓ Ride updated.\n");
    }
    ...
    void updateEvent()
    {
        if (events.empty())
        {
            cout << "No events available to update.\n";
            return;
        }

        listEvents();
        int eventId = readNumber<int>("Enter Event ID to update: ", 1, 1000000);
        Event *event = findByIdMutable(events, eventId);
        if (!event)
        ...
        if (choice == 3 || choice == 4)
        {
            int newCapacity = readNumber<int>("Enter new capacity: ", 1, 10000);
            if (newCapacity < event->getRegistered())
                throw runtime_error("Capacity cannot be less than registered visitors.");
            event->setCapacity(newCapacity);
        }

        ConsoleColor::printSuccess("✓ Event updated.\n");
    }
    ...
    void updateInventory()
    {
        if (inventory.empty())
        {
            cout << "No inventory items available to update.\n";
            return;
        }

        listInventory();
        int itemId = readNumber<int>("Enter Inventory ID to update: ", 1, 1000000);
        Inventory *item = findByIdMutable(inventory, itemId);
        if (!item)
            throw runtime_error("Inventory ID not found.");

        cout << "1. Update Name\n";
        cout << "2. Update Quantity\n";
        cout << "3. Update Cost per Item\n";
        cout << "4. Update All\n\n";
        int choice = readNumber<int>("Enter choice: ", 1, 4);
        ...
        if (choice == 2 || choice == 4)
        {
            int newQuantity = readNumber<int>("Enter new quantity: ", 0, 1000000);
            item->setQuantity(newQuantity);
        }

        if (choice == 3 || choice == 4)
        {
            double newCost = readNumber<double>("Enter new cost per item: ", 0.0, 10000.0);
            item->setCostPerItem(newCost);
        }

        ConsoleColor::printSuccess("✓ Inventory item updated.\n");
    }
    ...
    void updateStaff()
    {
        if (staff.empty())
        {
            cout << "No staff members available to update.\n";
            return;
        }

        listStaff();
        int staffId = readNumber<int>("Enter Staff ID to update: ", 1, 1000000);
        Staff *member = findByIdMutable(staff, staffId);
        if (!member)
        ...
        if (choice == 3 || choice == 4)
        {
            double newSalary = readNumber<double>("Enter new monthly salary: ", 0.0, 100000.0);
            member->setSalary(newSalary);
        }

        ConsoleColor::printSuccess("✓ Staff member updated.\n");
    }
    ...
    void updateVisitor()
    {
        if (visitors.empty())
        {
            cout << "No visitors available to update.\n";
            return;
        }

        listVisitors();
        int visitorId = readNumber<int>("Enter Visitor ID to update: ", 1, 1000000);
        Visitor *visitor = findByIdMutable(visitors, visitorId);
        if (!visitor)
        ...
        if (choice == 2 || choice == 4)
        {
            int newAge = readNumber<int>("Enter new age: ", 1, 120);
        ...
        ConsoleColor::printSuccess("✓ Visitor updated.\n");
    }
    ...
    void updateTicket()
    {
        if (tickets.empty())
        {
            cout << "No tickets available to update.\n";
            return;
        }

        listTickets();
        int ticketId = readNumber<int>("Enter Ticket ID to update: ", 1, 1000000);
        Ticket *ticket = findByIdMutable(tickets, ticketId);
        if (!ticket)
            throw runtime_error("Ticket ID not found.");

        cout << "1. Update Visitor\n";
        cout << "2. Update Ride\n";
        cout << "3. Update Amount\n";
        cout << "4. Update All\n\n";
        int choice = readNumber<int>("Enter choice: ", 1, 4);
        ...
        if (choice == 2 || choice == 4)
        {
            ...
            ticket->setAmount(ride->getPrice());
        ...
        if (choice == 3 || choice == 4)
        {
            double newAmount = readNumber<double>("Enter new ticket amount: ", 0.0, 10000.0);
            ticket->setAmount(newAmount);
        }

        ConsoleColor::printSuccess("✓ Ticket updated.\n");
    }
